...
export function WorkspaceEmpty({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed p-8 text-center sm:p-12">
      ...
      <p className="text-muted-foreground max-w-sm text-pretty">{description}</p>
      {action}
    </div>
  );
}
...
export function WorkspaceError({
  ...
  return (
    <div
      role="alert"
      className="flex flex-col items-center gap-3 rounded-xl border border-red-300 bg-red-50 p-8 text-center sm:p-12 dark:border-red-900 dark:bg-red-950/30"
    >
      <TriangleAlert className="size-9 text-red-600 dark:text-red-400" aria-hidden />
      <h2 className="text-xl font-semibold text-red-900 dark:text-red-200">
        Something went wrong
      </h2>
      <p className="max-w-md text-pretty text-red-800 dark:text-red-300">{message}</p>
      {onRetry && (
